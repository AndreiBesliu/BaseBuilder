...
export const MUTATII = [
  ...
  { n: 'capacitatea de transport ignorata', f: 'src/sim/joburi.ts',
    ...
    e2: [{ f: 'src/sim/joburi.ts', a: "    const cant = Math.min(liberPeItem(w, s), rules.haulCarryMax)\n    const prioLoc", b: "    const cant = liberPeItem(w, s)\n    const prioLoc" }],
    t: 'tests/carat.test.ts', e: 'capacitatea de transport LEAGA' },
  // --- logistica constructiei, commit 1: rezervari cu o singura forma pe CARAT (25.09.2026) ---
  { n: 'al doilea caraus cere cat e in morman, nu cat e LIBER (75 nu se imparte 50 + 25)', f: 'src/sim/joburi.ts',
    a: "      const cant = Math.min(liberPeItem(w, s), rules.haulCarryMax)\n      if (!poateRezerva", b: "      const cant = Math.min(it.cantitate[s]!, rules.haulCarryMax)\n      if (!poateRezerva",
    t: 'tests/carat.test.ts', e: 'doi carausi pe un morman de 75' },
  { n: 'liberPeItem citeste stratul LUCRU in loc de CARAT (suma rezervata e mereu zero)', f: 'src/sim/joburi.ts',
    a: "  return w.iteme.cantitate[is]! - sumaRezervata(w.rezervari, w.iteme.id[is]!, Strat.CARAT)", b: "  return w.iteme.cantitate[is]! - sumaRezervata(w.rezervari, w.iteme.id[is]!, Strat.LUCRU)",
    t: 'tests/carat.test.ts', e: 'doi carausi pe un morman de 75' },
  { n: 'mutaItem: fragmentul VIU cu mai putin nu reconciliaza (rezervarile il cred intreg)', f: 'src/sim/joburi.ts',
    a: "  if (nou !== -1 && it.cantitate[nou]! >= cant) return", b: "  if (nou !== -1) return",
    t: 'tests/carat.test.ts', e: 'un morman care cade si se contopeste PARTIAL' },
