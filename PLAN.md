...
|---|---|---|---|---|
| **D1** | **Motorul: DESCHIS prin decizie, redus la două — Unity 6.3 LTS + C# vs. TS/three.js/Electron. Gate măsurat la finalul S8 (luna 3).** Ales de owner, sesiunea 2 | **deschisă deliberat, cu gate măsurat** | Panoul de motor a scos **pluginurile de teren din decizie**: toți cei trei avocați au ajuns independent la „scrie mesher-ul singur" (godot_voxel e ostil la C#, Voxel Plugin 2 se auto-declară buggy, Voxelica e sculpting smooth, Ultimate Terrains e abandonat). Ce rămâne ca criteriu: **ergonomia UI-ului dens** (~70% din UX-ul unui colony sim, și singura axă pe care expertiza React/TS se transferă direct), rampa de limbaj, testabilitatea headless și livrarea pe Steam. Judecătorul de motor zice Unity, condiționat de arhitectura de grilă cubică — care e exact D19. Arhitectura câștigătoare își propune însă TS/Electron și **își declară singură slăbiciunea**: zero precedent livrat în gen, iar Vampire Survivors (Phaser+Electron) a migrat pe Unity la 1.6 pentru performanță. **Nucleul e portabil în ambele cazuri** (vezi D2), deci schimbarea costă doar stratul de randare. | **protocolul e scris și comis: [`bench/GATE.md`](bench/GATE.md).** Trei corecții față de formularea de aici: (a) „60 FPS" e saturație sub vsync, nu măsurătoare — metrica e X_max, bugetul liber, prin bisecție; (b) mașina de dezvoltare (7950X + RTX 3060) e de 2–5× peste clasa țintă, deci **nu poate semna STAY**, doar STAY-PROVIZORIU; (c) „costă 2 luni" e o estimare făcută de partea cu preferință — se înlocuiește cu un spike de portare cronometrat de 2 zile |
...
| **D20** | **Așezarea liberă a clădirilor: POZĂ liberă, COLIZIUNE pe grilă** — o clădire are centru în milimetri și orientare liberă (vector unitate Q14, nu unghi); ce se desenează e dreptunghiul rotit, ce se simulează sunt celulele pe care le acoperă | **deschisă — propunere măsurată, așteaptă owner-ul** | Cererea („nu-mi place că totul este construit din pătrate") lovește direct în D19, care e închisă și unanimă: grila de 1 m e universală **ca sistem de coordonate ȘI DE COLIZIUNE**, iar organicul trăiește **exclusiv în prezentare**. D20 nu răstoarnă D19 — mută poza, nu coliziunea. Riscul real nu e estetic, e că **un zid rotit curge**: arată continuu pe ecran și are goluri în grilă, deci agenții trec prin pereți dintr-o cauză pe care n-ai găsi-o niciodată privind zidul. Măsurat, nu presupus (`src/sim/amprenta.ts`, `tests/amprenta.test.ts`): cu regula **ORICE atingere**, zidul e etanș la toate unghiurile 0–180°; cu regula **centrul celulei** curge, iar un zid de 0,2 m așezat exact pe o graniță de celulă blochează **zero** celule — eșec tăcut și total. Costul etanșeității scade cu mărimea: hală 12×7 m → de 1,24–1,33× aria reală (neglijabil); colibă 4×3 m → de 2×; zid subțire → 1,13 celule/m la 0° și 2,38 la 45°, adică scara inerentă oricărei grile. Consecință de design care iese direct din cifre: **axa subțire a unui zid se ancorează pe centrul celulei** (20 de celule → 10, la aceeași lungime), chiar dacă poziția pe lungime și unghiul rămân libere. | dacă owner-ul vrea coliziune cu adevărat off-grid — atunci cade D19, nu D20 |
...
| **S6-8** | **Coloana vertebrală de randare.** Mesher binary greedy pe măști `Uint32` per coloană ✅; viewer three.js cu slice view și picking ✅; streaming de chunkuri cu buget pe cadru + traversare pe șine ✅; **fundația de suprafață** ✅ — zgomot Q14 (rezoluția verticală 17,6 cm → 1,1 cm), cote de vârf corecte, convenția de cotă într-un singur loc, o paletă și o lege de lumină pentru ambele suprafețe. **tăierea fețelor de la granița de chunk** ✅ (−5,3% quaduri, cost nedecis); **sonda de cadre + cele trei scenarii de gate** ✅. Rămâne: netezirea geometrică *(amânată deliberat — vezi DEVLOG)* și mesher-ul în worker. | 0 |
| | 🚦 **GATE DE MOTOR (D1).** Protocol **pre-înregistrat** în [`bench/GATE.md`](bench/GATE.md), comis înainte de prima cifră, cu predicție sigilată și program de verdict (`tools/gate-verdict.mjs`). Criteriul NU mai e „60 FPS" — sub vsync e saturație, nu măsurătoare. E **X_max**, bugetul liber în ms, găsit prin bisecție. Cinci ieșiri: REFUZ / SCOPE / FAIL-CANDIDAT / GREY / STAY-PROVIZORIU. | |
| **S9-11** | **Regiuni + reachability.** Partiție 16×16 per z-slice ✅, graf de regiuni ✅, reachability prin union-find ✅, fuzz de 10.000 de edituri ✅. **Reconstrucție incrementală** ✅ — costa ~30 ms și creștea cu rezidența; acum **2,8 ms la 600 de blocuri** și nu mai crește deloc. Graf de adiacență + re-etichetare prin BFS, în locul union-find-ului care nu putea desface o unire. Overlay de debug ✅ — și a găsit, în prima privire, că acoperirea verticală a lui `ensureArea` era greșită pe pantă. | 1 |
| **S12-15** | **Agenți și coliziune.** A* pe graf de regiuni + A* local ✅; **regula de ostilitate (D7)** ✅ — inclusiv D7c, care dă `OCUPAT_DE_OSTIL`, nu `INACCESIBIL`. **Măsurat:** un refuz costă **0,5 µs** față de 380 µs un drum de 109 celule — de ~700× mai ieftin, și ăsta e tot rostul stratului de regiuni. Rămâne: agenții care chiar merg pe drum, plafonul de re-planificări, acceptanța de 100k ticks. | 1 |
| **S16-19** | **Joburi și nevoi.** Priorități pe tipuri de muncă, job giver cu buckets spațiale, hauling, zone pictate, foame/odihnă/dispoziție. *Prima dată când devine joc.* **Tăietura 1 livrată (15.09):** desemnări de săpat → pull → scor continuu în întregi → rezervare-tuplu → loc de lucru (K01) → săpat pe aceeași cale ca `dig`; cauze pe desemnare și pe pion; overlay K13 (`J`); schema 2 cu fixtură golden. Designul a trecut printr-un panou adversarial înainte de cod, care a schimbat trei lucruri (plafonul de scan sortat pe margine, răcirea pe scopul predicatului, celula de lucru refăcută); apoi codul printr-o recenzie cu 6 lentile care a mai schimbat opt (`dirty` persistat, răcirea pe pereche ca mulțime, hoinăreala în blocuri legate + coridoare de acoperire, locul de lucru pe componentă, poarta de facțiune, scanarea de după job, refacerea amânată pe regiuni stale, memorarea ambelor refuzuri scumpe). **Tăietura 2 livrată (15.09):** săpatul produce mormane (`digYield` ca tabel în content), depozite pictate ca dreptunghi (o comandă, o zonă, celule cu id de entitate), cărat în patru pași cu două rezervări (sursa până la RIDICA, destinația până la LASA), `jobCantitate` înghețat în job, o singură rutină de depunere (`asazaItem`, pierdere numărată), cârlige la editarea terenului, index DERIVED de zone (`libere`, `acceptante[fel]`, `deMutat`) ca răspuns la K05, răciri pe ce EXISTĂ după refuz (zona, mormanul lăsat jos); schema 4 cu fixtură golden de schema 3. Panoul de design (5 lentile, 56 de constatări) a schimbat opt lucruri înainte de cod. Apoi codul a trecut printr-o recenzie cu 3 lentile (23 de constatări), care a găsit o **buclă infinită** — INACCESIBIL nu număra nicio încercare, iar re-alegerea țintei întorcea aceeași celulă — plus șapte găuri mai mici, toate reparate cu gardă și mutație. Măsurat: raportul drum/lucru 4,7 în carieră+depozit (semnalul pentru batching); scenariul standard 74 µs/tick. **Tăietura 3 livrată (15.09):** nevoile ca tabel indexat (foame, odihnă), cu DOUĂ ceasuri — scurgere rară, dar verificarea pragurilor la fiecare tick, fiindcă „liber ȘI pe tic de nevoie" e o coincidență de ~1 la 250; nevoile ca PRAG, nu ca scor care concurează cu munca (verificat în sursa primară); `prag` = preferă, `pragCritic` = întrerupe, dar numai când are ce face — altfel răcire pe perechea (pion, nevoie) și înapoi la muncă. Două feluri noi de job (MANANCA, DOARME) printr-un **tabel de drivere** care a înlocuit cinci `else` ce presupuneau SAPA: fără el, fiecare pion care mănâncă sau doarme la salvare ar fi pierdut jobul la încărcare. Dispoziția ca țintă DERIVED (suma gândurilor) peste o bară PERSISTED, cu rate asimetrice și pas limitat la distanța rămasă; gânduri de STARE recalculate vs. de EVENIMENT stocate într-o mulțime mărginită. Trei trepte de consecință: lucrează mai încet (podeaua pe REZULTAT, nu pe factor), refuză munca, pleacă. Schema 5 și 6, ambele cu migrare care umple explicit și DEFAZAT pe id — zero nu e valoare neutră. 315 teste, 89 din 89 de mutații, scenariul standard 79–81 µs/tick, hash `e6585927`. Panoul de design (5 lentile, 60 de constatări, 20 critice) a schimbat patru rădăcini înainte de cod. Măsurat după: raportul drum/lucru a urcat de la 4,7 la **7,7**; **8 din 32 de pioni pleacă** dintr-o colonie aprovizionată, fiindcă hoinăreala-substitut din S12-15 îi scoate din raza aprovizionării (OWNER_VERIFY 8). Rămâne: producția de hrană; batching-ul cărat-ului; ancorarea hoinărelii. | 1 |
| **S20-23** | **Construcție și stabilitate.** Blueprints, materiale, multi-etaj, scări, acoperișuri; propagare de suport (4 / −1 / max 3 nesusținute / grinzi 10). Test literal: **pivnița 7×7 se prăbușește, 5×5 nu.** **Tăietura 1 livrată (16.09):** regula ca **definiție, nu recurență** — `suport(c) = max(0, suportMax − d(c))`, unde `d` e distanța minimă laterală prin solid până la un voxel *așezat*, calculată ca BFS mărginit peste mulțimea surselor. Scrisă ca recurență („suportul vecinului minus 1"), panoul a rulat-o în trei ordini pe același teren și a obținut **trei hărți, niciuna corectă** — 10 din 25 de celule greșite, și 1 voxel prăbușit în loc de 9. Constanta `suportMax = 4` e fixată **unic** de perechea (5 ține, 7 cade); v1 scria pentru jucător „camere mai late de 5 nu stau singure", ceea ce e fals — măsurat, **6×6 ține**, iar cine sapă o sală de 6 și nu pățește nimic nu mai crede regula niciodată. Ancora = sub fereastra de voxeli **a coloanei**, sau în afara lumii: citită prin `materialAt` ar fi dat 841 din 1024 de voxeli cu suport 0 pe nivelul de bază al unui chunk **neatins** (~7.500 căzuți la prima săpătură, care promovează 9 chunkuri), și ar fi rupt camerele care traversează o graniță — `zBaseM` diferă pe **86,8%** dintre perechile vecine, maxim 18 m. Asta **e** K16. **Fără hartă de suport:** varianta DERIVED sub steag murdar dă 237.408 intrări / 144,6 ms pentru cele 9 chunkuri ale unei singure săpături și 1.332.035 / 1297 ms la 49 — iar numărul de chunkuri promovate **nu scade niciodată** (K05). Interogarea pură costă **0,058 µs** pe un voxel așezat și **4,345 µs** pe unul atârnat, și atârnați sunt 12 în tot scenariul standard. Invalidarea = **două** discuri Manhattan de rază 3, la z **și** z+1 (50 de celule): v1 avea „discul de la z plus un voxel deasupra", care prinde 1 din cele 9 celule ce ajung la 0. Voxelul căzut devine AER și lasă `MOLOZ` pe prima celulă cu ceva solid dedesubt — solid, blochează, e **el însuși sprijin**, deci cascada se oprește prin masă, nu doar prin geometrie; varianta „devine aer și cade marfă" făcea prăbușirea o recompensă. Căderea nu are plafon de **pas**: `maxStepM` refolosit ca plafon de cădere pierde 100% din marfă la 2+ niveluri și lasă pionul în aer pe veci. Legea prăbușirii, măsurată: **(W−6k)² voxeli la z+k**, total ≈ W³/18 — 7×7 dă 1 voxel, și e nevoie de 145×145 pentru o pâlnie. **Previzualizarea se face pe MULȚIMEA desemnărilor**, nu pe comandă: la 7×7 sunt 49 de comenzi, fiecare evaluată pe lumea neatinsă, deci **0 din 49** avertizează și tavanul crapă la săpătura 46, a unui pion pe care jucătorul nu-l urmărea (K07 în formă pură). Overlay-ul (tasta `S`) desenează cifra **tavanului**, nu a celulei active — 0% dintre voxelii nivelului activ au altceva decât 4 — și arată **acțiuni**, nu un gradient: *sigur* / *ultima celulă* / *cade*. 337 de teste, 28 de mutații noi (117 în total), hash `95dafb4f`. Panoul de design (5 lentile, 46 de constatări, 17 critice) a rescris designul înainte de cod, iar o **recenzie adversarială** (6 lentile, 59 de agenți, 21 de constatări confirmate de câte doi verificatori independenți) a găsit după aceea patru lucruri pe care cele 21 de mutații verzi nu le puteau vedea: `cellKey` nu e injectiv în afara lumii, deci o săpătură la marginea de vest ștergea o rocă la 16 km est; `stareSapat` se uita la suportul voxelului de deasupra, nu la ce cade — într-o cameră **dreptunghiulară** de 6×13, toate cele 216 celule spuneau SIGUR și 14 chiar prăbușeau ceva; overlay-ul desena peste planul de tăiere și cu slice-ul oprit lua nivelul din altitudinea camerei; iar prăbușirea curăța doar vârful coloanei, lăsând pioni zidiți în moloz și marfă pe celule dispărute — asta a mutat hash-ul, și **34 de coloniști vii în loc de 30** în scenariul standard. Lecția: o mutație probează că testele leagă codul pe care îl **ating fixturile**; toate fixturile erau pătrate, iar într-o cameră pătrată ce cade chiar *e* voxelul de deasupra. Rămâne: blueprints, materiale, scări, acoperișuri, grinzi; verificarea la **plasare** (`fill` nu trece prin regulă); memorarea previzualizării (32 ms la 400 de desemnări). **Tăietura 3 livrată (25.09):** logistica construcției — rezervări cu cantitate pe morman (`itemClaimantsMax`, `liber = Q − Σcount`), ridicarea în mai multe rânduri (prag per piesă, a doua sursă pe drumul curent → morman → șantier, retintire la moartea sursei), costul sondei mărginit de munca cerută, nu de mormanele din lume. Măsurat: 3 constructori pe un morman de 75 din 220–420 de tickuri în banda celor trei mormane (112–120); 30 de mormane de 10 fără depozit din 0 pereți în 3; scenariul standard 88–92 µs/tick. Trei defecte pre-existente găsite de panou reparate în aceeași felie (munca falsă, întreruperea cu mâna plină, fragmentul la cădere). Panoul de design (5 lentile, 62 de constatări) a răsturnat rădăcina lui v2 înainte de cod: `maxCount = Q vie` în tuplul rezervării era fals azi pe două căi măsurate. **Recenzia adversarială a codului (25.09, 6 lentile + 2 verificatori, 49/13):** sonda și retintirea aveau doi predicați (componentă, rază, prag) — bucla ridică-lasă la 5 mormane sigilate, ±50 nezidit, 15 + 5 nepornit; acum un predicat (`liberPentru`) și un lanț (`lantAcopera`), rază 2R de la șantier; oracolul de cantitate exclude comestibilele; șantierul ocupat se așteaptă; 14 gărzi fără probă au primit-o. Rămân în registru: consolidarea la cărat (celula cu același fel), 2·dMin pe relief (452 vs 204), `refaSursa` pe regiuni murdare. **Tăietura 4 livrată (25.09): grinda.** Regula stratificată: o grindă e *activă* dacă e ea însăși ținută de ceva așezat (`s0 > 0`, fără înlănțuire), iar `s1 = suportRazaGrinda − d1` până la cea mai apropiată grindă activă; `suport = max(s0, s1)`. Panoul de design (3 lentile, 24 de constatări, 5 CRITIC/MARE) a găsit două CRITIC-uri înainte de cod: mulțimea care cade se calcula DUPĂ `dig`, cu grinda deja scoasă din index, iar activitatea fără memorie costa 215–246 ms pe o săpătură. Măsurat pe codul livrat: fâșia din zid 3 → 12, placa peste un stâlp 25 → 181 → 313, sala 6×6 → 23×23 cu 4 grinzi (529/529 stau); o grindă la 400 de celule nu costă nimic (0 interogări `s1`); săpătura lângă grinzi 7–19 ms, stâlpul pe 5 etaje 25 ms pentru 1578 de voxeli; hash `52b16ed2` neschimbat. Overlay-ul S costă 3,2 s pe trecere lângă 4 grinzi (26 ms fără), deci s-a feliat pe cadre (4 ms/cadru). Viewer-ul alege piesa (P). 28 de probe noi; S6e (grinda care se dezactivează fără să cadă) a închis patru ratări. Rămân în registru: overlay-ul pe câmpuri (16–20 ms), `d1` ca câmp per propagare, precizia ULTIMA_CELULA, grinzile în tavanele de rocă. La owner: OWNER_VERIFY 12. **Recenzia adversarială a grinzii (26.09): 5 lentile + 12 verificatori, 34 de constatări, 10/10 verificate reale.** Reparate: poarta scanerului (tick în repaus 763 → 0,26 ms pe o sală cu podea de grinzi), închiderea (0,58 s la 41×41×3, era 7,5–9,9 s), previzualizarea pe mai multe săpături (depindea de ordinea lor), prefiltrul la margine, overlay-ul care se relua fără editări, „De ce nu?" (mințea pe cazul-vitrină), grinda prin material, raza plafonată la 16; plus un defect M5 pre-existent (un save între o comandă de teren și tick diverga) și harnașamentul de mutații, care lăsase o mutație în arbore raportând „restaurat". 22 de mutații plauzibile treceau suita — au acum teste. 521 de teste, 361 de probe. **Tăietura 5 livrată (26.09): accesul vertical.** Măsurat pe HEAD: nimic peste g+2 (zidul de 4, 6/12; tavanul, 0), un pion pe creasta camerei fără ușă pe 3/3 semințe, casa cu două etaje oprită la 53/194 — iar previzualizarea le promitea. Trei designuri, două panouri (13,78 M + 3,9 M tokeni; 22 + 8 CRITIC/MARE confirmate). Livrat: atingerea +2 și diagonala la zidire; locul de lucru SIGUR — graful stabil (calcabil acum și la finalul planului, mulțime care doar crește cât se execută planul) și componenta deschisă; regula de sigilare pe W; previzualizarea pe același predicat, simulată până la punct fix, cu `faraAcces` și cauza (scară / ușă); SCARA de piatră; desenarea la nivelul activ. Casa cu două etaje: 193/193, 0 pioni blocați. 563 de teste, 407 probe. La owner: OWNER_VERIFY 13. | 2 |
...
| **M5** | Save/load versionat | Envelope `{saveVersion, schema, gameVersion, timestamp, data}`. Fiecare câmp etichetat PERSISTED / DERIVED / TRANSIENT. Migrări în lanț idempotente cu fixture-uri golden în repo. Scriere atomică temp → fsync → rename, cu `.bak`. ~~**La load: anulează toate job-urile și toate rezervările**, lasă pionii să replanifice.~~ **Rafinat la S16-19 (15.09):** joburile sunt PERSISTED, rezervările DERIVED din ele și reconstruite la load în ordinea slotului; ce nu se poate re-rezerva se anulează **cu raport**. Anularea oarbă la load ar fi rupt invariantul `1000 + save + load + 1000 == 2000` — un pion cu jobul anulat reia progresul de la zero, altă poziție, alt hash. Intenția (un save inconsistent nu deadlock-uiește tăcut) se păstrează. |
...
|---|---|---|
| K01 | Pathfinding 3D multi-agent + ordonarea job-urilor — groapa fără fund a genului. Un studio de 10 oameni l-a rescris de 4 ori în 5 ani și după 1.0 îl declară în continuare problema #1 | prima dată când scriu cod care caută o poziție de lucru liberă în jurul unei ținte. **DECLANȘAT 15.09:** `celulaDeLucru` în `src/sim/joburi.ts` — 30 de linii, ordine fixă, și panoul de design a găsit în ele exact cascada anunțată aici (locul de lucru pe capacul voxelului vecin). Reparat înainte de cod; se ține sub ochi |
...
| K05 | Itemele de pe jos — bomba cu întârziere | TPS-ul scade cu VECHIMEA coloniei, nu cu populația. **S-a declanșat de două ori la tăietura 2 (15.09) și a fost măsurat de fiecare dată:** (1) un coridor de acoperire ancorat de poziția unui pion făcea acoperirea de regiuni să crească nemărginit (4972 de blocuri stabile → 24.350 și în creștere la 30.000 de tickuri, `relabel` 40% din tick) — coridoarele spre marfă au fost scoase, garda e „zero blocuri noi în 2000 de tickuri"; (2) poarta ieftină întreba „e loc?" în loc de „încape cât car?", iar plafonul de căutare a destinației nu lega niciodată. **Rămâne deschis:** reconstrucția indexului de zone e Θ(celule + iteme) — măsurat 20,9 µs la 0 mormane, 135,4 µs la 3000, deci ~0,5 ms/tick la ținta din DESIGN §10. Se face incrementală când cifra din joc o cere; garda de acceptanță numără PAȘI (12/tick azi, prag 30), nu reconstrucții. |
